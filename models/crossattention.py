...
class AttentionBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor, mask: torch.Tensor = None, return_attn: bool = False):
        if return_attn:
            attn_out, attn_weights = self.attn(self.norm1(x), mask, return_attn=True)
...
class Attention(nn.Module):
    ...
    def forward(self, x: torch.Tensor, mask: torch.Tensor = None, return_attn: bool = False):
        ...
        if return_attn:
            ...
            attn_weights_drop = self.attn_drop(attn_weights) if self.training else attn_weights
            x = (attn_weights_drop @ v).transpose(1, 2).reshape(B, N, C)
            x = self.proj(x)
            x = self.proj_drop(x)
            return x, attn_weights  # attn_weights: [B, H, N, N]
